...
description: Describe cómo configurar la integración continua en Azure Pipelines mediante archivos Bicep. Muestra cómo usar un script de PowerShell o copiar archivos en una ubicación de almacenamiento provisional e implementar desde allí.
author: mumian
ms.topic: conceptual
ms.author: jgao
ms.date: 06/01/2021
...
ms.locfileid: "111528529"
---
# <a name="integrate-bicep-with-azure-pipelines"></a>Integración de Bicep con Azure Pipelines

Los archivos Bicep se pueden integrar con Azure Pipelines para la integración e implementación continuas (CI/CD). En este artículo, aprenderá a compilar un archivo Bicep en una plantilla de Azure Resource Manager (plantilla de ARM) y, a continuación, a usar dos formas avanzadas de implementar plantillas con Azure Pipelines.
...
* **Uso de una tarea de la CLI de Azure** Use esta tarea para ejecutar `az bicep build` para compilar los archivos Bicep antes de implementar las plantillas de ARM.

* **Usar la tarea de implementación de plantilla de ARM**. Esta opción es la más sencilla. Este enfoque funciona cuando se desea implementar una plantilla de ARM directamente desde un repositorio. Esta opción no se trata en este artículo, sino que se trata en el tutorial [Integración continua de plantillas de ARM con Azure Pipelines](../templates/deployment-tutorial-pipeline.md). Muestra cómo usar la [tarea de implementación de plantilla de ARM](https://github.com/microsoft/azure-pipelines-tasks/blob/master/Tasks/AzureResourceManagerTemplateDeploymentV3/README.md) para implementar una plantilla desde el repositorio de GitHub.
...
* Tiene una [plantilla de Resource Manager](../templates/quickstart-create-templates-use-visual-studio-code.md) que define la estructura del proyecto.
...
En esta sección se muestra cómo compilar un archivo Bicep en una plantilla de ARM antes de implementar la plantilla.

El siguiente archivo YAML compila un archivo Bicep mediante una [tarea de la CLI de Azure](/azure/devops/pipelines/tasks/deploy/azure-cli):
...
Para `scriptType`, use **bash**.

Para `scriptLocation`, use **inlineScript** o **scriptPath**. Si especifica **scriptPath**, también deberá especificar un parámetro `scriptPath`.

En `inlineScript`, especifique las líneas de script.  El script proporcionado en el ejemplo compila un archivo Bicep llamado *azuredeploy.bicep* que existe en la raíz del repositorio.
...
Al establecer la tarea en `AzurePowerShell@5`, la canalización usa el [Módulo Az](/powershell/azure/new-azureps-module-az).
...
* `deploymentScope`: seleccione el ámbito de implementación entre las opciones: `Management Group`, `Subscription` y `Resource Group`.
...
La siguiente canalización muestra cómo compilar un archivo Bicep y cómo implementar la plantilla compilada:
