...
| Mensaje de error | Causas posibles | Acción recomendada |
|---|---|---|
| La operación se canceló porque ya se estaba ejecutando una operación en conflicto en la misma base de datos. | Los siguientes son los casos en los que puede que surja este código de error:<br><ul><li>Se agregan o quitan archivos en una base de datos mientras se está haciendo una copia de seguridad.</li><li>Se están reduciendo los archivos mientras se están haciendo copias de seguridad de la base de datos.</li><li>Está en curso una copia de seguridad de la base de datos por parte de otro producto de copia de seguridad configurado para la base de datos, y la extensión de Azure Backup desencadena un trabajo de copia de seguridad.</li></ul>| Deshabilite el otro producto de copia de seguridad para resolver el problema.
...
|---|---|---|
| La copia de seguridad, las operaciones de manipulación de archivos (como ALTER DATABASE ADD FILE) y los cambios de cifrado de una base de datos se deben serializar. | Este error puede producirse cuando el trabajo de copia de seguridad desencadenado a petición o programado tiene conflictos con una operación de copia de seguridad que ya se está ejecutando activada por la extensión de Azure Backup en la misma base de datos.<br> Los siguientes son los escenarios en los que puede aparecer este código de error:<br><ul><li>Se está ejecutando una copia de seguridad completa en la base de datos, y se desencadena otra copia de seguridad completa.</li><li>Se está ejecutando una copia de seguridad diferencial en la base de datos, y se desencadena otra copia de seguridad diferencial.</li><li>Se está ejecutando una copia de seguridad de registros en la base de datos, y se desencadena otra copia de seguridad de registros.</li></ul>| Una vez que se produzca un error en la operación en conflicto, reinicie la operación.
