...
Cuando se usa el protocolo AMQP, que es el protocolo exclusivo para clientes de .NET Standard, Java, JavaScript, Python y Go, y que es [una opción para el cliente de .NET Framework](service-bus-amqp-dotnet.md), las transferencias de mensajes y las liquidaciones se canalizan y se vuelven completamente asincrónicas. Se recomienda usar las variantes de la API del modelo de programación asincrónica.
...
La estrategia para administrar el resultado de las operaciones de envío puede tener un impacto inmediato y significativo en el rendimiento para la aplicación. Los ejemplos de esta sección están escritos en C# y se aplican a futuros de Java, monos de Java, promesas de JavaScript y conceptos equivalentes en otros lenguajes.
...
El modo **receive-and-delete** indica al agente que considere todos los mensajes que envía al cliente receptor como liquidados cuando se envían. Esto significa que el mensaje se considera consumido tan pronto como el agente lo pone en circulación. Si se produce un error en la transferencia del mensaje, el mensaje se pierde.
...
El modo **peek-lock** indica al agente que el cliente receptor desea liquidar explícitamente los mensajes recibidos. El mensaje ahora está disponible para que el receptor lo procese, mientras se mantiene en un bloqueo exclusivo en el servicio para que los demás receptores competidores no puedan verlo. La duración del bloqueo se define inicialmente en el nivel de cola o suscripción y la puede ampliar el cliente que posee el bloqueo. Para más información sobre la renovación de bloqueos, consulte la sección [Renovación de bloqueos](#renew-locks) de este artículo. 
...
Cuando los receptores liberan repetidamente el mensaje o permiten el bloqueo transcurra durante un número definido de veces ([Número máximo de entregas](service-bus-dead-letter-queues.md#maximum-delivery-count)), el mensaje se quita de la cola o suscripción automáticamente y se coloca en cola de mensajes fallidos asociada.

El cliente receptor inicia la liquidación de un mensaje recibido con una confirmación positiva cuando llama a la API `Complete` para este mensaje. Indica al agente que el mensaje se ha procesado correctamente y el mensaje se ha quitado de la cola o suscripción. El agente responde al intento de liquidación del receptor con una respuesta que indica si se pudo realizar la liquidación.

Cuando el cliente receptor no puede procesar un mensaje pero quiere que el mensaje se vuelva a entregar, puede pedir explícitamente que el mensaje se libere y se desbloquee al instante mediante una llamada a la API `Abandon` para el mensaje o bien puede no hacer nada y dejar que el bloqueo transcurra.

Si un cliente receptor no puede procesar un mensaje y sabe que volver a enviar el mensaje e reintentar la operación no va a ser la solución, puede rechazar el mensaje, que lo traslada a la cola de mensajes fallidos mediante una llamada a la API `DeadLetter` en el mensaje. Este método también permite establecer una propiedad personalizada con un código de motivo que se puede recuperar con el mensaje procedente de la cola de mensajes fallidos.

Un caso especial de liquidación es el aplazamiento, que se explica en [otro artículo](message-deferral.md).

Las operaciones`Complete`, `Deadletter` o `RenewLock` pueden producir un error debido a problemas de red, si el bloqueo mantenido ha expirado o hay otras condiciones del lado del servicio que impiden la liquidación. En uno de los últimos casos, el servicio envía una confirmación negativa que se muestra como una excepción en los clientes de API. Si el motivo es una conexión de red rota, se quita el bloqueo, ya que Service Bus no admite la recuperación de los vínculos AMQP existentes en una conexión diferente.

Si `Complete` produce un error, que surge normalmente al final del control de mensajes y, en algunos casos, después de algunos minutos de trabajo de procesamiento, la aplicación receptora puede decidir si se conserva el estado del trabajo y pasa por alto el mismo mensaje cuando se envía una segunda vez, o si elimina el resultado del trabajo y lo vuelve a intentar cuando el mensaje se reenvía.
...
## <a name="renew-locks"></a>Renovación de bloqueos
El valor predeterminado para la duración del bloqueo es **30 segundos.** Puede especificar un valor diferente para la duración del bloqueo en el nivel de cola o suscripción. El cliente propietario del bloqueo puede renovar el bloqueo del mensaje mediante métodos en el objeto receptor. En su lugar, puede usar la característica de renovación automática de bloqueos, donde puede especificar el tiempo durante el que desea seguir renovando el bloqueo. 

## <a name="next-steps"></a>Pasos siguientes
- Un caso especial de liquidación es el aplazamiento. Consulte [Aplazamiento de mensajes](message-deferral.md) para obtener más información. 
- Para obtener información sobre las colas de mensajes fallidos, vea [Colas de mensajes fallidos](service-bus-dead-letter-queues.md).
- Para más información sobre la mensajería de Service Bus en general, consulte [Colas, temas y suscripciones de Service Bus](service-bus-queues-topics-subscriptions.md).