---
title: Notas del producto que describen conceptos de Azure Cosmos DB
description: Obtenga la lista de notas del producto de Azure Cosmos DB, estas notas del producto describen los conceptos en profundidad.
ms.service: cosmos-db
ms.subservice: cosmosdb-sql
author: SnehaGunda
ms.author: sngun
ms.topic: conceptual
ms.date: 05/07/2021
ms.custom: seodec18
ms.openlocfilehash: 3f26f83dcaeb2bd2cfbdc6545ee092bb053d22a8
ms.sourcegitcommit: ba8f0365b192f6f708eb8ce7aadb134ef8eda326
ms.translationtype: HT
ms.contentlocale: es-ES
ms.lasthandoff: 05/08/2021
ms.locfileid: "109634066"
---
# <a name="azure-cosmos-db-whitepapers"></a>Notas del producto de Azure Cosmos DB
[!INCLUDE[appliesto-all-apis](includes/appliesto-all-apis.md)]

Las notas del producto permiten explorar los conceptos de Azure Cosmos DB en un nivel más profundo. En este artículo se proporciona una lista de las notas del producto disponibles para Azure Cosmos DB.

| **Notas del producto** | **Descripción** |
| --- | --- |
|[Indexación independiente del esquema con Azure Cosmos DB](https://www.vldb.org/pvldb/vol8/p1668-shukla.pdf) | En este documento se describe el subsistema de indexación de Azure Cosmos DB. En este documento se incluyen funcionalidades de Azure Cosmos DB, como la representación de documentos, el lenguaje de consulta, el enfoque de indexación de documentos, la compatibilidad de índices principales y las primeras experiencias de producción.|
| [Azure Cosmos DB y los datos personales](https://servicetrust.microsoft.com/ViewPage/TrustDocuments?command=Download&downloadType=Document&downloadId=87cc6456-4b23-473c-94d3-6c713b8b8956&docTab=6d000410-c9e9-11e7-9a91-892aae8839ad_FAQ_and_White_Papers)| En este documento se ofrece orientación para los clientes de Azure Cosmos DB que administran una base de datos basada en la nube, una base de datos local o ambas y que necesitan asegurarse de que los datos personales contenidos en sus sistemas de bases de datos se controlan y protegen de conformidad con la normativa en vigor. |
|[Desarrollo de aplicaciones de nueva generación con Azure Cosmos DB](https://azure.microsoft.com/resources/microsoft-azure-cosmos-db-flexible-reliable-cloud-nosql-at-any-scale/) | En este documento se explora cómo Azure Cosmos DB se coloca de forma única para abordar los requisitos de datos de las aplicaciones modernas. Incluye tres casos de clientes, donde se destacan la API para MongoDB, la administración simplificada de datos, la escalabilidad rentable, el rendimiento líder del mercado y la confiabilidad. |
