...
Puede habilitar esta extensión mediante cmdlets de Azure PowerShell, scripts de la CLI de Azure, plantillas de Resource Manager (plantillas de ARM) o Azure Portal. Para obtener más información, consulte [Extensiones y características](features-linux.md).
...
>
>Para evitar los conflictos en tiempo de instalación, use la [directiva `dependsOn`](../../azure-resource-manager/templates/define-resource-dependency.md#dependson) para instalar las extensiones de manera secuencial. Las extensiones se pueden instalar en cualquier orden.
...
El archivo ejecutable `python2` debe tener el valor *python* como alias. Esta es una manera de configurar el alias:
...
```
#### <a name="azure-cli-sample-for-installing-lad-40-on-a-virtual-machine-scale-set-instance"></a>Ejemplo de la CLI de Azure para la instalación de LAD 4.0 en la instancia del conjunto de escalado de máquinas virtuales
...
* En las VM con el modelo de implementación clásica, especifique la versión `4.*` si va a instalar la extensión mediante la CLI xPlat de Azure o PowerShell.
...
1. Realice las selecciones como se describió anteriormente.
...
Copie la SAS generada en el campo `storageAccountSasToken`. Quite el signo de interrogación inicial (?).
...
La extensión Diagnostics de Linux 4.0 admite dos tipos de receptores: `EventHub` y `JsonBlob`.
...
Si no quiere habilitar syslog o la recopilación de métricas, especifique una estructura vacía para el elemento `ladCfg`, como en este ejemplo: 
...
scheduledTransferPeriod | Frecuencia con la que se computan las métricas agregadas y se transfieren a Métricas de Azure Monitor. La frecuencia se expresa como un intervalo de tiempo ISO 8601. El período de transferencia mínimo es de 60 segundos, es decir, PT1M. Especifique al menos un `scheduledTransferPeriod`.
...
* Número de muestras sin procesar usadas para procesar el agregado
...
DisplayName | Etiqueta que se agregará a los datos en Métricas de Azure Monitor. Esta etiqueta se establece en el idioma especificado por la configuración regional asociada. LAD omite este campo.
...
* `/builtin/Disk/FreeSpace`: espacio libre promediado entre todos los sistemas de archivos montados

Ni LAD ni Azure Portal esperan que el valor de `counterSpecifier` coincida con ningún patrón. Sea coherente con la forma en que se construyen los valores `counterSpecifier`.

Si especifica `performanceCounters`, LAD siempre escribe datos en una tabla en Azure Storage. Los mismos datos se pueden escribir en blobs JSON o instancias de Event Hubs, o ambos. Sin embargo, no puede deshabilitar el almacenamiento de datos en una tabla. 
...
La característica de métricas de Azure Monitor de Azure Portal usa los datos de esta tabla para crear gráficos o desencadenar alertas. El nombre de tabla es la concatenación de las siguientes cadenas:
...
Si especifica `syslogEvents`, LAD siempre escribe datos en una tabla en Azure Storage. Los mismos datos se pueden escribir en blobs JSON o instancias de Event Hubs, o ambos. Sin embargo, no puede deshabilitar el almacenamiento de datos en una tabla. 
...
En la sección `sinksConfig` opcional se habilita el envío de métricas al receptor de Azure Monitor, además de la cuenta de almacenamiento y la hoja Métricas de invitado predeterminada.
...
table | (Opcional) Tabla de Azure Storage en la que se escriben nuevas líneas de la "cola" del archivo. La tabla debe estar en la cuenta de almacenamiento designada, como se especificó en la configuración protegida. 
...
En una máquina virtual con dos vCPU, si una está ocupada al 100 % y la otra está inactiva al 100 %, el valor de `PercentIdleTime` notificado es 50. Si la actividad de cada vCPU es del 50 % durante el mismo período, el resultado indicado también es de 50. En una máquina virtual de cuatro vCPU, cuando una está ocupada al 100 % y las otras están inactivas, el valor de `PercentIdleTime` notificado es 75.
...
LAD no expone métricas de ancho de banda. Estas se pueden obtener a partir de métricas de host.

Contador | Significado
------- | -------
BytesTransmitted | Total de bytes enviados desde el inicio
BytesReceived | Total de bytes recibidos desde el inicio
BytesTotal | Total de bytes enviados o recibidos desde el inicio
...
La clase de métricas File system proporciona información acerca del uso del sistema de archivos. Los valores absolutos y porcentuales se notifican como se mostrarían a un usuario normal (no raíz).
...
## <a name="install-and-configure-lad-40"></a>Instalación y configuración de LAD 4.0
...
El comando supone que se está usando el modo de Administración de recursos de Azure de la CLI de Azure. Si quiere configurar LAD para las máquinas virtuales con el modelo de implementación clásica, cambie al modo "asm" (`azure config mode asm`) y omita el nombre del grupo de recursos en el comando. 
...
* Un punto de conexión de Event Hubs, tal y como se especificó en el receptor de `EventHubs`.
