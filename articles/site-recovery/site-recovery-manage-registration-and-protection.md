...
6. Si el servidor de VMM estaba en estado desconectado, descargue y ejecute el [script de limpieza](unregister-vmm-server-script.md) en el servidor VMM. Abra PowerShell con la opción **Ejecutar como administrador** para cambiar la directiva de ejecución para el ámbito predeterminado (LocalMachine). En el script, especifique el ID del servidor VMM que desea quitar. El script quita el registro y la información de emparejamiento de nube del servidor.
